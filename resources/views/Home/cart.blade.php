                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle p-4">
                                            <select id="number_course_{{$j}}"
                                                    onchange="set_number_course('{{ $key }}','{{ $j }}')"
                                                    class="form-control text-center">
                                                @for($i=1;$i<31;$i++)
                                                    <option @if($value2==$i) selected="selected" @endif
                                                    value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                        </td>
                                        ...
                                        <td class="text-right font-weight-semibold align-middle p-4">
                                            {{number_format($data['price2']* $value2)}}
                                            تومان
                                        </td>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center pb-4">
                    ...
                    <div class="d-flex">
                        <div class="text-right mt-4">
                            ...
                            <label class="text-muted font-weight-normal m-0">قیمت کل دوره :
                                <strong>{{number_format($total_price)}}</strong>

                                تومان
                            </label>
                            <br>
                            <label class="text-muted font-weight-normal m-0">مقدار تخفیف :
                                <strong>{{number_format($total_price - $price)}}</strong>
                            ...
                            @endphp
                            <label class="text-muted font-weight-normal m-0">قیمت پرداختی شما :
                                <strong>{{number_format($price2)}}</strong>
...
@endphp
                <div class="float-left">
                    <form action="{{route('cart.payment')}}" id="cart-payment" method="post">
                        @csrf
                     <input type="hidden" name="course_id[]" value="{{json_encode($cart)}}">
...
@section('js')
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data == 'error') {
                            ...
                            $("#discount_box").hide();
                            $("#loading").html(html);
                            window.location.replace(data ? "/cart" : "/cart");
        ...
        del_course = function (c_id) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    window.location.replace(data ? "/cart" : "/");
                }
            });
        };
    ...
    <?php
    $url2 = route('site.ajax_change_cart');
    ?>
    <script>
