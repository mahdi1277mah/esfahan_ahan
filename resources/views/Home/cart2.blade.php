                        ...
                        @foreach(\Cart::instance('cart-faranesh')->all() as $cart)
                            @if(isset($cart['course']))
                                ...
                                <tr>
                                    ...
                                    <td class="text-right font-weight-semibold align-middle p-4">{{ $course->price }} تومان</td>
                                    <td class="align-middle p-4">
                                        <select onchange="changeQuantity(event, '{{ $cart['id'] }}' , 'cart-faranesh')" class="form-control text-center">
                                            @foreach(range(1 , $course->inventory) as $item)
                                                <option value="{{ $item }}" {{  $cart['quantity'] == $item ? 'selected' : '' }}>{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="text-right font-weight-semibold align-middle p-4">تومان {{ $course->price * $cart['quantity'] }}</td>
                                    {{--                                <td class="text-center align-middle px-0">--}}
                                    {{--                                    <form action="{{ route('cart.destroy' , $cart['id']) }}" id="delete-cart-{{ $course->id }}" method="post">--}}
                                    {{--                                        @csrf--}}
                                    {{--                                        @method('delete')--}}
                                    {{--                                    </form>--}}
